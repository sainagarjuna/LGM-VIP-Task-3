...
const displayEntries = () => {
  ...
  const rows = entries
    .map((entry) => {
      ...
      const gender=`<td class="td">${entry.gender}</td>`;
      const img=`<td class="td"><img src=${entry.img} alt='image' style="width:100px;height:100px" /></td>`;
      const acceptConditions = `<td class="td">${entry.acceptConditions}</td>`;

      const row = `<tr>${name} ${email} ${password} ${dob} ${gender} ${img} ${acceptConditions}</tr>`;
      return row;
    })
    .join("\n");
